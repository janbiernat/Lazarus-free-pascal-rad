...
(*--== SQLite3 for Lazarus ==--
  ...
  Usunięcie wybranego rekordu z bazy.
...
begin
  //Usuń wybrany rekord.
  TakNie:= 0;
  TakNie:=  Application.MessageBox('Czy usunąć wybrany rekord?',
                                   'Usuń rekord', MB_ICONQUESTION + MB_YESNO);
  if(TakNie = IDYES) then begin
    try
      SQLite3Connection1.Close;
      SQLite3Connection1.DatabaseName:= tSciezka[1];
      SQLite3Connection1.Open;
      SQLTransaction1.Active:= TRUE;
      SQLQuery1.Clear;
      SQLQuery1.SQL.Text:= 'DELETE FROM "sklep" WHERE id2 = :id2';
      SQLQuery1.Params.ParamByName('id2').AsString:= Trim(StringGrid1.Cells[1, StringGrid1.Row]);
      SQLQuery1.ExecSQL;
      SQLTransaction1.Commit;
    finally
    end;
...
begin
  ...
  try
    ...
    SQLTransaction1.Active:= TRUE;
    SQLQuery1.Clear;
    SQLQuery1.SQL.Text:= 'UPDATE "sklep" '+
                         'SET artykul = :artykul, '+
                             'datawaz = :datawaz, '+
                             'cena = :cena '+
                         'WHERE id2 = :id2';
...
begin
  //Aktualizacja: Przenieś dane do pól edycyjnych.
  with StringGrid1 do begin
    Edit2.Text:= Trim(Cells[2, Row]);
    MaskEdit1.Text:= Trim(Cells[3, Row]);
    Edit3.Text:= Trim(Cells[4, Row]);
    Enabled:= FALSE;
  end;
end;
