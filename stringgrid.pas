...
type { TForm1 }
  ...
  public
    ...
    function StringGridCzyElementJest(StringGridN:TStringGrid; Kol:Integer; Str:String):Integer;
...
function TForm1.StringGridCzyElementJest(StringGridN:TStringGrid; Kol:Integer; Str:String):Integer;
var I, _Wynik:Integer;
begin //StringGridCzyElementJest - Sprawdza, czy podany element znajduje się w wybranej kolumnie.
  I:= 0; _Wynik:= 0;
  Str:= Trim(AnsiLowerCase(Str));
  if(Str<>'') then begin
    with StringGridN do begin
      for I:= 0 to RowCount-2 do begin
        if(AnsiLowerCase(Trim(Cells[Kol, (I+1)])) = Str) then begin
          _Wynik:= (I+1); Break;
        end;
      end;
    end;
...
begin //FormCreate.
  ...
  Button9.Caption:= 'Czy jest imię "Jan"?';
...
begin
  W:= 0;
  W:= StringGridCzyElementJest(StringGrid1, 3, 'GÓrsKA 165a');
  ShowMessage('Wiersz nr: '+IntToStr(W)+' | '+Trim(StringGrid1.Cells[3, W]));
end;
